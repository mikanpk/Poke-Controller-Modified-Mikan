...
class ImageProcPythonCommandAddin(ImageProcPythonCommand):
    ...
    #     if len(crop) == 4:
    #         src = src[crop[1]: crop[3], crop[0]: crop[2]]
    #     src = self.edgeFilter(src)

    #     # テンプレート画像の読み取り
    #     template = cv2.imread(_get_template_filespec(template_path), cv2.IMREAD_COLOR)
    ...
    #     # テンプレート画像の読み取り
    #     template = self.edgeFilter(template)
    ...
    #     # cv2.drawContours(img_edges, contours, -1, (255, 0, 0), 2, cv2.LINE_AA)
    #     cv2.rectangle(src, (max_loc[0], max_loc[1]), (max_loc[0]+w, max_loc[1]+h), (255, 0, 255))  # ラベルを四角で囲む

    #     cv2.imshow('Canny Edge Detection', src)
    #     cv2.imshow('Canny template Detection', src)
    ...
    #     if crop != []:
    #         self.gui.ImgRect(
    #             crop[0], crop[1], crop[2], crop[3], outline="orange", tag=tag, ms=ms
    #         )
    #     if max_val >= threshold:
    #         if self.gui is not None and show_position:
    #             # self.gui.delete("ImageRecRect")
    ...
    # def edgeFilter(self, img_original):
    #     canny_lower = 100
    #     canny_upper = 200
    #     blurparam = 3
    ...
    #     img_edge = img_gray.copy()

    #     img_edge = cv2.medianBlur(img_edge, blurparam)
    #     # img_edge = cv2.Canny(img_edge, canny_lower, canny_upper)
    #     img_edge = cv2.Laplacian(img_edge, cv2.CV_64F, ksize=5)
...
#         i_x1 = np.maximum(x1[selected_index], x1[remaining_indices])
#         i_y1 = np.maximum(y1[selected_index], y1[remaining_indices])
#         i_x2 = np.minimum(x2[selected_index], x2[remainingaining_indices])
#         i_y2 = np.minimum(y2[selected_index], y2[remaining_indices])
...
#         # 座標 (0,0) に近い順にソート
#         distances = np.sqrt(selected_boxes[:, 0]**2 + selected_boxes[:, 1]**2)
#         sorted_by_distance = selected_boxes[np.argsort(distances)]
#         return sorted_by_distance
#     else:
#         # 一致率（スコア）の高い順にソート
#         sorted_by_score = selected_boxes[np.argsort(selected_scores)[::-1]]
